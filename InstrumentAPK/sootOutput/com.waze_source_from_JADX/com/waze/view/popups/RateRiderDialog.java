...
public class RateRiderDialog extends BaseBottomDialog {
    ...
    class C32023 implements OnClickListener {
        C32023() {
        }

        public void onClick(View v) {
            AnalyticsBuilder.analytics(AnalyticsEvents.ANALYTICS_EVENT_RW_RATE_RIDER_SHEET_SKIP_CLICKED).addParam(AnalyticsEvents.ANALYTICS_EVENT_INFO_RIDE_ID, RateRiderDialog.this.mRideId).send();
            RateRiderDialog.this.mRateRiderResult.onSkip();
            RateRiderDialog.this.dismiss();
        }
    }

    class C32034 implements OnCancelListener {
        C32034() {
        }

        public void onCancel(DialogInterface dialog) {
            AnalyticsBuilder.analytics(AnalyticsEvents.ANALYTICS_EVENT_RW_RATE_RIDER_SHEET_CANCELED).addParam(AnalyticsEvents.ANALYTICS_EVENT_INFO_RIDE_ID, RateRiderDialog.this.mRideId).send();
            RateRiderDialog.this.mRateRiderResult.onSkip();
        }
    }
    ...
    public RateRiderDialog(Context context, String rideId, CarpoolUserData rider, RateRiderResult onRes) {
        super(context);
        this.mRideId = rideId;
        this.mRider = rider;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        srv.setListener(new StarRatingListener() {
            public void onRatingChanged(int rating) {
                AnalyticsBuilder.analytics(AnalyticsEvents.ANALYTICS_EVENT_RW_RATE_RIDER_SHEET_STAR_CLICKED).addParam(AnalyticsEvents.ANALYTICS_EVENT_INFO_RIDE_ID, RateRiderDialog.this.mRideId).addParam(AnalyticsEvents.ANALYTICS_EVENT_INFO_STARS, "" + rating).send();
                if (rating > 0) {
        ...
        tvRate.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                AnalyticsBuilder.analytics(AnalyticsEvents.ANALYTICS_EVENT_RW_RATE_RIDER_SHEET_DONE_CLICKED).addParam(AnalyticsEvents.ANALYTICS_EVENT_INFO_RIDE_ID, RateRiderDialog.this.mRideId).addParam(AnalyticsEvents.ANALYTICS_EVENT_INFO_STARS, "" + rating).send();
                RateRiderDialog.this.mRateRiderResult.onRating(rating);
                RateRiderDialog.this.dismiss();
            }
        });
        tvSkip.setOnClickListener(new C32023());
        setOnCancelListener(new C32034());
    }
}
