...
public class UnregisterStatusNotificationCommand extends WLServiceCommand {
    ...
    public static final short ID = (short) 113;
    ...
    public UnregisterStatusNotificationCommand(DataBuffer $r1) throws  {
        super($r1);
        if (!isValid()) {
            Log.e("UnregisterStatusNotificationCommand", "Not a valid command!");
        }
        short $s0 = getCommandID();
        if ($s0 != ID) {
            Log.e("UnregisterStatusNotificationCommand", "Invalid command ID: " + $s0);
        }
    }

    public UnregisterStatusNotificationCommand(String $r1, String $r2, int $i0) throws  {
        super(ID, 4, $r1, $r2);
        if (super.isValid()) {
