...
public class Scroller {
    ...
    static {
        ...
        for (int i = 0; i <= 100; i++) {
            ...
            float x_max = 1.0f;
            while (true) {
                x = x_min + ((x_max - x_min) / LayoutManager.WAZE_LAYOUT_EDIT_SIDE_MARGIN);
                coef = (3.0f * x) * (1.0f - x);
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY) {
        ...
        this.mDuration = (int) (1000.0d * Math.exp(l / (((double) DECELERATION_RATE) - FriendsBarFragment.END_LOCATION_POSITION)));
        this.mStartTime = AnimationUtils.currentAnimationTimeMillis();
        this.mStartX = startX;
        this.mStartY = startY;
        float coeffX = velocity == 0.0f ? 1.0f : ((float) velocityX) / velocity;
        float coeffY = velocity == 0.0f ? 1.0f : ((float) velocityY) / velocity;
        int totalDistance = (int) (((double) ALPHA) * Math.exp((((double) DECELERATION_RATE) / (((double) DECELERATION_RATE) - FriendsBarFragment.END_LOCATION_POSITION)) * l));
        this.mMinX = minX;
        this.mMaxX = maxX;
        this.mMinY = minY;
        this.mMaxY = maxY;
